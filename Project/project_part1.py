...
class Game():
    ...
    def __init__(self, queue):
        ...
        self.preyCoordinates = ()   # variable to track location of prey
        self.createNewPrey()
    ...
    def superloop(self) -> None:
        ...
        while self.gameNotOver:
            ...
            self.move()         # move snake
            time.sleep(SPEED)   # sleep to allow for other tasks
    ...
    def move(self) -> None:
        ...
        caughtPrey = False  # flag for checking if prey has been caught

        if (NewSnakeCoordinates[0] >= self.preyCoordinates[0] and 
            NewSnakeCoordinates[1] >= self.preyCoordinates[1] and 
            NewSnakeCoordinates[0] <= self.preyCoordinates[2] and 
            NewSnakeCoordinates[1] <= self.preyCoordinates[3]):
            # check if start of snake is within prey coordinates

            self.score += 1                         # increment score
            self.queue.put({"score": self.score})   # put new score into queue
            self.createNewPrey()                    # generate new prey
            caughtPrey = True                       # set flag so snake length can increase

        # check for game over
        self.isGameOver(NewSnakeCoordinates)

        self.snakeCoordinates.append(NewSnakeCoordinates)   # add new coordinate to snake
        if caughtPrey == False:
            del self.snakeCoordinates[0]                    # if no prey caught, remove first coordinate to "move" snake
        else:
            caughtPrey == False                             # if prey is caught, toggle flag

        self.queue.put({"move": self.snakeCoordinates})     # put new coordinates into queue
    ...
    def calculateNewCoordinates(self) -> tuple:
        ...
        if self.direction == "Left":    # snake moving left
            return (lastX - 10, lastY)
        elif self.direction == "Right": # snake moving right
            return (lastX + 10, lastY)
        elif self.direction == "Down":  # snake moving down
            return (lastX, lastY + 10)
        elif self.direction == "Up":    # snake moving up
            return (lastX, lastY - 10)
        else:
            return (lastX, lastY)       # default value in case of error (should never be reached)
    ...
    def isGameOver(self, snakeCoordinates) -> None:
        ...
        if x <= 0 or x >= WINDOW_WIDTH or y <= 0 or y >= WINDOW_HEIGHT or snakeCoordinates in self.snakeCoordinates:
            # check if snake has exceeded window dimensions or overlaps itself
            self.gameNotOver = False                        # set gameNotOver flag to false
            self.queue.put({"game_over": self.gameNotOver}) # put flag into queue
    ...
    def createNewPrey(self) -> None:
        ...
        # set random coordinates in multiples of 10 to align with snake movement 
        x = random.randrange(THRESHOLD, WINDOW_WIDTH - THRESHOLD + 1, 10)   
        y = random.randrange(THRESHOLD, WINDOW_HEIGHT - THRESHOLD + 1, 10)

        self.preyCoordinates = (x - 5,  y - 5, x + 5, y + 5)    # set prey coordinates
        self.queue.put({"prey": self.preyCoordinates})          # put prey into queue
