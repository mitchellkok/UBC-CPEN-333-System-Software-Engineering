...
def philosopher(id: int, chopstick: list): 
    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        def thinkForAWhile(): #simulates  thinking time with a random delay
        ...
        #to simplify, try statement not used herephilosopher

        while(1):   # loop until both chopsticks are successfully picked up
            acquiredLeft = chopstick[leftChopstick].acquire(block=False)
            if acquiredLeft:    # left chopstick successfully picked up

                acquiredRight = chopstick[rightChopstick].acquire(block=False)
                if acquiredRight: # both chopsticks successfully picked up
                    break   # break loop when both chopsticks are acquired

                # release right chopstick on failure
                chopstick[leftChopstick].release()

            # release left chopstick on failure
